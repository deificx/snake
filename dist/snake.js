...
/***/ function(module, exports, __webpack_require__) {
	...
	var _animationFrame = __webpack_require__(2);
	...
	var _snake = __webpack_require__(9);

	var _snake2 = _interopRequireDefault(_snake);

	var _keypress = __webpack_require__(10);
	...
	var textHeight = 50;
	var textWidth = _html2.default.width - textMarginLeft * 2;
	var render = function render() {
		...
		context.fillRect(0, 0, _html2.default.width, _html2.default.height);
		...
		switch (s.getState()) {
			...
			case _constants.ENDED:
				...
				if (s.getScore() > 0 && s.getScore() >= highScore) {
					...
					context.fillText("Congratulations, new high score: " + highScore, textMarginLeft, textHeight * 3, // eslint-disable-line
					textWidth);
				} else {
					context.fillText("You scored " + s.getScore() + " points. Your high score is " + highScore, textMarginLeft, textHeight * 3, // eslint-disable-line
					textWidth);
				}
				break;
			default:
	...
	listener.simple_combo('up', function () {
		s.setDir(_constants.UP);
	});

	listener.simple_combo('right', function () {
		s.setDir(_constants.RIGHT);
	});

	listener.simple_combo('down', function () {
		s.setDir(_constants.DOWN);
	});

	listener.simple_combo('left', function () {
		s.setDir(_constants.LEFT);
	});
...
/***/ function(module, exports) {
	...
	var SCORE_INCREASE = exports.SCORE_INCREASE = 10;
...
/***/ function(module, exports, __webpack_require__) {
	...
	Snake.prototype.randomDir = function () {
		...
		switch (Math.floor(Math.random() * cases)) {
			case 0:
				this.direction = _constants.UP;
				return _constants.UP;
			case 1:
				this.direction = _constants.LEFT;
				return _constants.LEFT;
			case 2:
				this.direction = _constants.DOWN;
				return _constants.DOWN;
			default:
				this.direction = null;
				return null;
		}
	...
	Snake.prototype.update = function (dt) {
		...
		if (this.wait > 0 || this.state !== _constants.STARTED) {
			return;
		}
		...
		if (move.x === this.apple.x && move.y === this.apple.y) {
			...
			this.position = move;
			this.score = this.score + _constants.SCORE_INCREASE;
		} else if (this.collision(move)) {
			this.state = _constants.ENDED;
		} else {
			this.snake.pop();
			this.snake.unshift(move);
			this.position = move;
